...
#define CHANGE_OPTION_LIST_TABLE_3_PARAM_NUM 4

static ChangeOptListTable change_option_list_table_3[CHANGE_OPTION_LIST_TABLE_3_PARAM_NUM] =
{
	{"Collate", "StapleCollate", "CNOutputPartition", "offset",		 NULL, NULL, "CNOffsetNum", "1"},
	{"Collate", "StapleGroup",   "CNOutputPartition", "offset",		 NULL, NULL, "CNOffsetNum", "1"},
	{"Collate", "Group",         "CNOutputPartition", "offset",		 NULL, NULL, "CNOffsetNum", "1"},
	{NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL}
};

#define CHANGE_OPTION_LIST_TABLE_LIST_NUM 4
	...
	if( opt_len > 0 )
	{
		...
		if( (p_job_attr = malloc(opt_len + 1)) != NULL )
		{
			...
			if( strstr( p_job_attr, "CN_Filter_DebugMode" ) != NULL ) {
				g_in_debug_fd = open( "/tmp/debug_image.ps", O_WRONLY|O_CREAT|O_TRUNC, S_IWUSR );
			}
		}
	}
	return p_job_attr;
	...
	if( pipe(fds) >= 0 )
	{
		...
		if( child_pid == 0 )
		{
			...
			if( g_in_debug_fd > 0 ) {
				...
				char * const timeStr = ctime(&timer);

				FILE *fp_log = NULL;
				unsigned int loop_cnt= 0;

				fp_log = fopen("/tmp/debug_optionlist.log", "w");
				if( fp_log != NULL ) {
					fprintf(fp_log, "\n%s", timeStr);
					for( loop_cnt = 0; cmd_arg[loop_cnt] != NULL; loop_cnt++ ) {
						fprintf(fp_log, "cmd_arg[%d] : %s\n", loop_cnt, cmd_arg[loop_cnt]);
					}
					fclose(fp_log);
				}
			}
...
int	main(int argc, char *argv[])
{
	...
	cmd_arg = init_cmd_arg(org_cmd_arg, ARGV_NUM, &cmd_arg_num);
	if (cmd_arg == NULL) {
		fputs("ERROR: can't init parameter.\n", stderr);
		goto error_return;
	}
...
#endif
		if( g_in_debug_fd > 0 ) {
			buflist_write(p_ps_data, g_in_debug_fd);
		}
...
#endif
				if( g_in_debug_fd > 0 ) {
					write(g_in_debug_fd, p_data_buf, read_bytes);
				}
#ifdef	DEBUG_IN_PS
				if( g_in_log_fd > 0 )
